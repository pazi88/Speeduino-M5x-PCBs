...
// initialize timer1 to send CAN data in 30Hz rate (about)
  noInterrupts();
  TCCR1A = 0;
  TCCR1B = 0;
...
void requestData() {
  ...
  Serial.write(-ResponseLength +25); //offset for the data. 7 with CLT, 14 without
  Serial.write(0x00); // offset is in 2 bytes. LSB first
  Serial.write(ResponseLength);	//how mony bytes we need back.
...
void loop() {
 if (Serial.available () > 0) {  // read bytes from serial3
